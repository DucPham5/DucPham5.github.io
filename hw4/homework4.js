...
async function loadStates() {
    try {
        ...
        const savedState = localStorage.getItem("cmc_state");
        if (savedState) document.getElementById("state").value = savedState;
    } catch (error) {
...
function saveField(fieldId) {
    ...
    if (["ssn", "password", "confirmPassword"].includes(fieldId)) return;
    const el = document.getElementById(fieldId);
    if (el) localStorage.setItem("cmc_" + fieldId, el.value);
}
...
function saveCheckboxes() {
    ...
    localStorage.setItem("cmc_conditions", JSON.stringify(checked));
}

function saveRadio(groupName) {
    const selected = document.querySelector(`input[name="${groupName}"]:checked`);
    if (selected) localStorage.setItem("cmc_" + groupName, selected.value);
}
// ── Restore all fields from localStorage ────────────────
function restoreFromLocalStorage() {
    const textFields = ["firstName", "middleInitial", "lastName", "dob", "phone",
                        "address1", "address2", "city", "state", "zip", "email",
                        "symptoms", "userid", "health"];
    textFields.forEach(id => {
        const saved = localStorage.getItem("cmc_" + id);
        const el = document.getElementById(id);
        if (saved !== null && el) el.value = saved;
    });

    // Health slider display
    const savedHealth = localStorage.getItem("cmc_health");
    if (savedHealth) document.getElementById("healthValue").textContent = savedHealth;

    // Checkboxes
    const savedConditions = localStorage.getItem("cmc_conditions");
    if (savedConditions) {
        const vals = JSON.parse(savedConditions);
    ...
    // Radios
    ["gender", "vaccinated", "insurance"].forEach(group => {
        const saved = localStorage.getItem("cmc_" + group);
        if (saved) {
            const el = document.querySelector(`input[name="${group}"][value="${saved}"]`);
...
function clearLocalStorage() {
    const keys = Object.keys(localStorage).filter(k => k.startsWith("cmc_"));
...
window.addEventListener("DOMContentLoaded", function() {
    ...
    const savedName = getCookie("patientFirstName");
    if (savedName) {
